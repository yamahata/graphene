...
#define ODEBUG(code, ms)                        \
    SGX_DBG(DBG_E, #code " ms: %p\n", ms)
#define ODEBUG_RV(code, rv)                     \
    SGX_DBG(DBG_E, #code " rv: %d\n", (rv))
#endif

static inline int linux_ret_to_pal(int ret)
{
...
static int sgx_ocall_futex(void * pms)
{
    ...
    SGX_DBG(DBG_E, "futext %p op %d val %d timeout %ld\n",
...
void * ocall_table[OCALL_NR] = {
        ...
        [OCALL_SCHED_GETAFFINITY] = (void *) sgx_ocall_sched_getaffinity,
        [OCALL_SCHED_SETAFFINITY] = (void *) sgx_ocall_sched_setaffinity,
