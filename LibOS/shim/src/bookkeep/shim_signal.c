...
void deliver_signal (PAL_PTR event, siginfo_t * info, PAL_CONTEXT * context)
{
    ...
    if ((preempt & ~SIGNAL_DELAYED) > 1 ||
        ...
        if (!__handle_signal(tcb, sig, &signal->context, event, context))
...
void handle_sysret_signal(void)
{
    shim_tcb_t * tcb = shim_get_tls();
    struct shim_thread * thread = (struct shim_thread *) tcb->tp;
    debug("sysret signal: regs %p stack: %p rip: %p syscall_nr: %ld rcx: 0x%lx\n",
          tcb->context.regs, tcb->context.sp, tcb->context.ret_ip,
          tcb->context.syscall_nr, tcb->context.regs? tcb->context.regs->rcx: 0);
